...
1. Compile first. This is a hard gate: if the build fails, stop immediately, report the build errors, and do not run any of the following steps.
...
- `pnpm lint` type-checks as well (oxlint runs with `typeCheck`), so it reports compiler diagnostics for files that `pnpm compile` does not cover, such as the scripts under `configs/`.
- The e2e suites are deliberately not part of this skill: they build and launch a Theia application, which takes minutes. Run `pnpm test:e2e` separately when changing anything under `e2e/` or the diagram integration itself.
